...
class ToyDatasets:
    ...
    def make_batch(self, batch_size: int, device) -> torch.Tensor:
        idx = torch.randint(
            low=0, high=self.dataset_len, size=(batch_size,), device=device
        )
    ...
    def sample(self, n: int, T: int = 4, temperature: float = 1.0) -> torch.Tensor:
        ...
        for i in range(T):
            ...
            for b in range(n):
                ...
                confidences = gumbel_logits[
                    b, masked_positions, candidate_tokens[b, masked_positions]
                ]
...
def run_one_dataset(dset_name):
    ...
    for step in rng:
        ...
        if (step + 1) % SAVE_EVERY == 0:
            ...
            total_loss = 0
            save_ckpt(
                last_ckpt, model, opt, step + 1, min(best_loss, avg_loss), seed=SEED
            )
